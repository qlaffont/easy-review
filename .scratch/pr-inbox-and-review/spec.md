...
2. As a reviewer, I want to reconnect or sign out, so that I can rotate the OAuth session or stop using the app on a shared machine.
...
33. As a hostable-app user, I want to open a deployed Easy Review origin, sign in with GitHub, and only ever see my GitHub data for that OAuth session, so that isolation does not require a multi-tenant database.
34. As a security-conscious user, I want my access token to stay in an HTTP-only cookie on the Easy Review origin (never in `localStorage` or client JS), so that XSS cannot read it directly.
...
43. As a reviewer, I want errors from GitHub (expired session, rate limit, missing scope) explained in the UI, so that I know how to fix access problems.
...
- **Auth**: GitHub OAuth App. Server stores the user access token in an HTTP-only cookie and attaches it when proxying. No personal access tokens, no device flow, no `gh` bridge.
